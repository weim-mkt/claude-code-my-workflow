...
Claude Code ships with built-in skills beyond this template's 53: `/batch` orchestrates parallel refactoring across your codebase (using git worktrees for isolation), `/simplify` runs 3-agent code review and applies fixes, and `/debug` helps troubleshoot sessions. These complement the academic skills above.
:::
...
The guide includes 53 [skills](#skills---reusable-slash-commands) for common academic tasks. But if you have repetitive workflows specific to your domain, you can create your own.
...
| `/codex` | `.claude/skills/codex/` | Drive the OpenAI Codex CLI for code analysis, refactoring, editing, or a plan-refinement debate (Claude and Codex iterate up to 5 rounds, or `--interactive`); critically evaluates Codex output as a peer |
