...
The template includes 8 hooks for logging, notifications, context survival, and git safety (plus a real git pre-commit hook, `.githooks/pre-commit`, installed via `./scripts/install-hooks.sh`):
...
| Code-path drift guard | `SessionStart` | Flags drift between referenced code paths and disk after a merge (paired with `.githooks/post-merge`) |
...
**Source-language coverage:** The pattern is language-agnostic. The template ships [`/data-analysis`](#all-skills) for R-first projects and [`/stata-replication`](#all-skills) (v1.9.0) for Stata-first projects (mirrors `/data-analysis` exactly; same numbered-pipeline shape, different language; executes via the `stata-mcp` MCP server). Python-first projects follow the same convention with the path `code/python/_outputs/`. For Stata users, [`stata-code-conventions.md`](.claude/rules/stata-code-conventions.md) (v1.9.0) codifies the header (`version 18`, `clear all`, `set seed`/`set sortseed`), numbered pipeline (00–99), `esttab` for `\input{}` tables, clustering discipline (`reghdfe`), balance via `iebaltab`, and AEA Data Editor compliance.
...
These skills produce structured reports saved to `quality_reports/`. The `/data-analysis` skill also generates R scripts (saved to `code/`) and runs the r-reviewer agent automatically.
...
| Writing Style | `writing-style.md` | No em dashes in generated prose (preventive companion to `/humanize`) |
...
| Content Invariants | `content-invariants.md` | `.tex`, `.qmd`, `Preambles/`, `code/**` |
...
| Stata Code Conventions | `stata-code-conventions.md` | `**/*.do`, `code/stata/**` — header scaffold, numbered pipeline, esttab tables, clustering discipline, AEA compliance (v1.9.0) |
...
| Code-path drift guard | SessionStart (command) + `.githooks/post-merge` | `.claude/hooks/check-code-path.sh` |
